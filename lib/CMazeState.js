...
export class CMazeState {
  ...
  getIdentifier() {
      const start = this.getCell(skEnterCol, skEnterRow);
      const br = this.getCell(skMazeCols - 2, skMazeRows - 2);
      const tr = this.getCell(skMazeCols - 2, 1);
      let cells  = [start, br, tr];
      let identifier = 0;
      for (let i = 0; i < cells.length; ++i) {
          if (cells[i].openTop) {
              identifier += 8 * Math.pow(16, i);
          }
          if (cells[i].openLeft) {
              identifier += Math.pow(16, i);
          }
          if (cells[i].openBottom) {
              identifier += 2 * Math.pow(16, i);
          }
          if (cells[i].openRight) {
              identifier += 4 * Math.pow(16, i);
          }
      }
